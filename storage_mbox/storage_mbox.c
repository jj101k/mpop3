...
int _storage_dump_message(struct pop3_message *message, FILE *out) {
	if(!message) return 1;
	return _storage_dump_fragment(message->header_offset, message->body_end_offset, 1, 0, out);
}

int _storage_dump_headers(struct pop3_message *message, FILE *out) {
	if(!message) return 1;
	return _storage_dump_fragment(message->header_offset, message->header_end_offset, 1, 0, out);
}
int _storage_dump_message_lines(struct pop3_message *message, unsigned long int max_lines, FILE *out) {
	if(!message) return 1;
...
struct pop3_message *_storage_message_number(unsigned long int index) {
	unsigned long int j;
	struct pop3_message *current_message;

	if(index>message_count) return NULL;
	if(index<1) return NULL;
	if(index==1) return first_message;
	current_message=first_message;
